...
class _ProductListState extends State<ProductList> {
  ...
  Future<void> _loadSelectedLanguage() async {
    ...
    String? selectedLanguage = await _storageService.getLang();
  ...
  Future<List<Product>> _loadProducts() async {
    ...
    final products = await ProductService().fetchProducts(context, lang);
    setState(() {
        ...
        categories = [
          {
            ...
            .map((item) => {
                  'id': item['id'],
                  'name': translate(item['name'], selectedLanguage)
                })
            .toList());
      });
    }
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        subCategories = [
          {
            'id': null,
            'name': translate('select_sub_category', selectedLanguage)
          }
        ]..addAll(data
            .map((item) => {
                  'id': item['id'],
...
Future<void> _deleteUser() async {
  ...
  void _updateFilterssizes(List<Product> products) {
    sizes = products.map((p) => p.taille ?? '').toSet().toList();
  }

  void _updateFiltersbrands(List<Product> products) {
    brands = products.map((p) => p.marque ?? '').toSet().toList();
  }
  ...
  void _filterProducts() {
    ...
    setState(() {
      filteredProducts = allProducts.where((product) {
        ...
        final matchesBrand = selectedBrand == null ||
            selectedBrand == translate('all', selectedLanguage) ||
            product.marque == selectedBrand;
        final matchesSize = selectedSize == null ||
            selectedSize == translate('all', selectedLanguage) ||
            product.taille == selectedSize;
    ...
    print(
        ("____________________________________________________________________________________________________________________________________________"));
    _updateFilters(filteredProducts);
  }

  void _filterProductsbrand() {
    String selectedLanguage =
        Provider.of<LanguageProvider>(context, listen: false).selectedLanguage;
    setState(() {
      filteredProducts = allProducts.where((product) {
    ...
    print(
        ("____________________________________________________________________________________________________________________________________________"));
    _updateFilterssizes(filteredProducts);
  }

  void _filterProductssize() {
    String selectedLanguage =
        Provider.of<LanguageProvider>(context, listen: false).selectedLanguage;
    setState(() {
      filteredProducts = allProducts.where((product) {
    ...
    print(
        ("____________________________________________________________________________________________________________________________________________"));
    _updateFiltersbrands(filteredProducts);
  }
  ...
  Future<void> _loadWishlistItems() async {
    ...
    if (user != null) {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          wishlistItems =
              wishlistData.map((item) => item['productId'] as int).toSet();
  ...
  Future<void> _removeFromFavorites(int productId) async {
    ...
    if (user == null) {
      ...
      if (response.statusCode == 200) {
        ...
        await _loadWishlistItems();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(translate("remove_wishlists", lang))),
        );
      } else {
  ...
  Future<void> _removeFromCarttt(int productId) async {
    ...
    if (user == null) {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(translate("remove_from_cart", lang))),
        );
      } else {
        print('Failed to remove from favorites: ${response.body}');
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _sendProductToEndpoint(int productId) async {
    ...
    if (user == null) {
      ...
      if (response.statusCode == 201) {
        setState(() {
          wishlistItems
              .add(productId); // Ajouter l'élément à la liste de souhaits
        });
        await _loadWishlistItems(); // Recharger toutes les données après l'ajout à la wishlist
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(translate("add_to_cartt", lang))),
        );
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content:
                  Text('Le produit est déjà dans votre liste de souhaits')),
        );
      }
    }
  }
  ...
  child: Text("Supprimez votre compte"),
                ...
                PopupMenuButton<String>(
                  onSelected: (String languageCode) async {
                    ...
                    // Recharger les catégories avec la nouvelle langue
                    if (selectedCategoryId != null) {
                      await _loadSubCategories(
                          selectedCategoryId!); // Recharger les sous-catégories avec la nouvelle langue
                    }
                  },
                    ...
                    return <PopupMenuEntry<String>>[
                      PopupMenuItem<String>(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color:
                                selectedLanguage == 'en' ? Colors.blue : null,
                          ),
                        ),
                      ),
                      PopupMenuItem<String>(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color:
                                selectedLanguage == 'fr' ? Colors.blue : null,
                          ),
                        ),
                      ),
                      PopupMenuItem<String>(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            color:
                                selectedLanguage == 'ar' ? Colors.blue : null,
                          ),
                        ),
                      ),
                    ];
                      ...
                      if (selectedCategoryId != null &&
                        ...
                        Expanded(
                          child: DropdownButton<int?>(
                            isExpanded: true,
                            hint: Text(translate(
                                'select_sub_category', selectedLanguage)),
                    ...
                    children: [
                      Expanded(
                        child: DropdownButton<String>(
                          isExpanded: true,
                          hint:
                              Text(translate('select_brand', selectedLanguage)),
                          value: selectedBrand,
                          onChanged: (value) {
                            setState(() {
                              selectedBrand = value;
                              _filterProductsbrand();
                            });
                          },
                          items: [
                            translate('select_brand', selectedLanguage),
                            ...brands
                          ].map((String brand) {
                      ...
                      Expanded(
                        child: DropdownButton<String>(
                          isExpanded: true,
                          hint:
                              Text(translate('select_size', selectedLanguage)),
                          value: selectedSize,
                          onChanged: (value) {
                            setState(() {
                              selectedSize = value;
                          ...
                          items: [
                            translate('select_size', selectedLanguage),
                            ...sizes
                          ].map((String size) {
                            return DropdownMenuItem<String>(
                              value: size,
                  ...
                  child: FutureBuilder<List<Product>>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return Center(
                            child: Text(
                                translate('no_products', selectedLanguage)));
                      } else {
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      padding: const EdgeInsets.symmetric(
                                          horizontal: 8.0),
                                      child: Text(
                                        '${product.price?.toStringAsFixed(2) ?? ''} ${translate('Mru', selectedLanguage)}',
                                        style: TextStyle(
